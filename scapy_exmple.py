...
print("\n----- Packet summaries --------------------")
capture = scapy.sniff(iface='Wi-Fi 2', count=10)
print(capture.nsummary())

# CAPTURE DNS AND PRINT PACKETS
print("\n----- DNS packet summaries (collect 10 DNS packets) --------------------")
capture = scapy.sniff(iface='Wi-Fi 2', filter="udp port 53", count=10)
print(capture.nsummary())
...
scapy.sniff(iface='Wi-Fi 2', prn=print_packet, filter="tcp port https", count=10)


# CAPTURE AND HANDLE PACKETS AS THEY ARRIVE Using LAMBDA
print("\n\n----- Capture and print packets as sniffed (using lambda) ---------------------")
scapy.sniff(iface='Wi-Fi 2', prn=lambda pkt: print(f"lambda    {pkt.summary()}"), filter="tcp port https", count=10)
